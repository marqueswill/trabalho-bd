...
class TesteProdutoLote(TesteBase):
    ...
    def test_insert(self):
        try:
            items = [
                ProdutoLote(codProduto=1, codEstoque=1, numLote=6, quantidade=10),
                ProdutoLote(codProduto=2, codEstoque=1, numLote=6, quantidade=20),
                ProdutoLote(codProduto=3, codEstoque=1, numLote=6, quantidade=30),
                ProdutoLote(codProduto=4, codEstoque=1, numLote=6, quantidade=40),
                ProdutoLote(codProduto=5, codEstoque=1, numLote=6, quantidade=50),
            ]
            for i in items:
    ...
    def test_delete_all(self):
        try:
            self.produtolote_db.delete_all()
            return "Success"
        except Exception as e:
            return f"Failed - {str(e)}"
