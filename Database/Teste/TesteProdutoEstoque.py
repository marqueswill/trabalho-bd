...
class TesteProdutoEstoque(TesteBase):
    ...
    def test_get_by_id(self):
        try:
            produtoEstoque = self.produtoEstoque_db.get_by_id(6, 2)
            if produtoEstoque:
                return "Success"
    ...
    def test_delete(self):
        try:
            pe = self.produtoEstoque_db.get_by_id(7, 2)
            self.produtoEstoque_db.delete(pe)
            return "Success"
        except Exception as e:
            return f"Failed - {str(e)}"
